-- lunarware | jjs (jujutsu shenanigans)
-- 25 features: auto farm, kill aura, esp players, esp mobs, esp bosses, esp items, teleport to player, teleport to boss, auto boss, auto collect items, speed boost, fly, infinite cursed energy, no cooldowns, auto dodge, auto block, auto combo, fullbright, no fog, third person, fov changer, hitbox extender, anti-afk, boss notifier, unlock moves

local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/peppy/UI-Library/main/source.lua"))()
local window = library:CreateWindow("lunarware | jjs")
...
-- boss list
local bosslist = {
    "sukuna", "gojo", "toji", "kenjaku", "mahito", "jogo", "hanami", "dagon"
}
...
local function autododge()
    ...
    if tick() - lastdodgetime < 2 then return end
            ...
            if humanoid then
                ...
                if animator then
                    for _, track in pairs(animator:GetPlayingAnimationTracks()) do
                        if track.Animation and (track.Animation.Name:lower():find("attack") or track.Animation.Name:lower():find("punch")) then
...
local function bossnotifier()
    if not settings.bossnotifier then return end
    ...
    for _, v in pairs(workspace:GetDescendants()) do
        if v:IsA("Model") then
            ...
            for _, b in pairs(bosslist) do
                if name:find(b) then
                    library:Notification("boss alert", b .. " has spawned!", "ok")
                end
            end
        end
    end
...
local function autocollectitems()
    ...
    for _, v in pairs(workspace:GetDescendants()) do
        if v:IsA("Model") and v:FindFirstChild("ClickDetector") then
            local name = v.Name:lower()
            if name:find("scroll") or name:find("item") or name:find("drop") or name:find("essence") then
                local char = localplayer.Character
                if char and char:FindFirstChild("HumanoidRootPart") then
...
local function setinfinitecursedenergy()
    if not settings.infinitecursedenergy then return end
    local character = localplayer.Character
    if not character then return end
    local energy = character:FindFirstChild("CursedEnergy")
...
-- unlock moves
local function unlockmoves()
    if not settings.unlockmoves then return end
    
    local movesfolder = replicatedstorage:FindFirstChild("Moves")
    if movesfolder then
        for _, move in pairs(movesfolder:GetChildren()) do
            for _, child in pairs(move:GetChildren()) do
...
farmingtab:CreateToggle("boss notifier", false, function(v) settings.bossnotifier = v end)
...
movementtab:CreateToggle("infinite cursed energy", false, function(v) settings.infinitecursedenergy = v end)
movementtab:CreateToggle("no cooldowns", false, function(v) settings.nocooldowns = v end)

-- misc tab
misctab:CreateToggle("hitbox expander", false, function(v) settings.hitboxsize = v end)
misctab:CreateSlider("hitbox size", 1, 3, 1, function(v) settings.hitboxsize = v end)
misctab:CreateToggle("anti-afk", false, function(v) settings.antiafk = v end)
misctab:CreateButton("unlock all moves (local)", function() unlockmoves() end)
